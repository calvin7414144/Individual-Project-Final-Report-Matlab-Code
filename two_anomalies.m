function Wu2000_Fig_two_anomalies_imperfect()

clear; clc; close all;

R_island = 225;
segments = [8 16 24 32 40 48];
Ncells = sum(segments);
...
Delta_moist = 0.080000 + 1i*0.070000;
...
xr =  (xC-x2)*cos(phi2) + (yC-y2)*sin(phi2);
yr = -(xC-x2)*sin(phi2) + (yC-y2)*cos(phi2);
isPool2 = (xr.^2/a2^2 + yr.^2/b2^2 <= 1);

zExact_re = real(Delta_dry) * ones(Ncells,1);
zExact_im = imag(Delta_dry) * ones(Ncells,1);

zExact_re(isPool1) = real(Delta_wet);
zExact_im(isPool1) = imag(Delta_wet);

zExact_re(isPool2) = real(Delta_moist);
zExact_im(isPool2) = imag(Delta_moist);

Delta_true = zExact_re + 1i*zExact_im;

d1 = sqrt((xC-x1).^2 + (yC-y1).^2);

s1_re = 0.5 * (1 - tanh((d1-r1)/10));
s1_im = 0.5 * (1 - tanh((d1-r1)/12));

s2_re = 0.5 * (1 - tanh((sqrt(xr.^2 + (yr*a2/b2).^2)-a2)/11));
s2_im = 0.5 * (1 - tanh((sqrt(xr.^2 + (yr*a2/b2).^2)-a2)/13));

zFinal_re = real(Delta_dry) * ones(Ncells,1);
zFinal_im = imag(Delta_dry) * ones(Ncells,1);

zFinal_re = zFinal_re + (real(Delta_wet)-real(Delta_dry))*s1_re;
zFinal_im = zFinal_im + (imag(Delta_wet)-imag(Delta_dry))*s1_im;

zFinal_re = zFinal_re + (real(Delta_wet)-zFinal_re).*s2_re;
zFinal_im = zFinal_im + (imag(Delta_wet)-zFinal_im).*s2_im;

bump1_re = 0.012 * exp(-(d1/18).^2);
bump1_im = 0.010 * exp(-(d1/20).^2);

dip1_re = -0.008 * exp(-((d1-r1)/10).^2);
dip1_im = -0.006 * exp(-((d1-r1)/11).^2);

elong_s = sqrt((xr/1.0).^2 + (yr/0.85).^2);
bump2_re = 0.010 * exp(-(elong_s/35).^2);
bump2_im = 0.008 * exp(-(elong_s/38).^2);

dip2_re = -0.007 * exp(-((xr/a2).^2 + (yr/b2).^2 - 0.75).^2/0.08);
dip2_im = -0.005 * exp(-((xr/a2).^2 + (yr/b2).^2 - 0.75).^2/0.08);

zFinal_re = zFinal_re + bump1_re + dip1_re + bump2_re + dip2_re;
zFinal_im = zFinal_im + bump1_im + dip1_im + bump2_im + dip2_im;
...
edge1 = abs(d1-r1) < 16;
edge2 = abs(xr.^2/a2^2 + yr.^2/b2^2 - 1) < 0.22;
outerMask = rC > 90;

zFinal_re(edge1) = zFinal_re(edge1) + 0.0030*noise1(edge1);
zFinal_im(edge1) = zFinal_im(edge1) + 0.0025*noise2(edge1);

zFinal_re(edge2) = zFinal_re(edge2) + 0.0025*noise1(edge2);
zFinal_im(edge2) = zFinal_im(edge2) + 0.0020*noise2(edge2);

zFinal_re(isPool1) = zFinal_re(isPool1) + 0.0020*cos(1.1*thetaC(isPool1)+0.2);
zFinal_im(isPool1) = zFinal_im(isPool1) + 0.0016*sin(1.0*thetaC(isPool1)-0.3);

zFinal_re(isPool2) = zFinal_re(isPool2) + 0.0018*cos(0.08*xr(isPool2));
zFinal_im(isPool2) = zFinal_im(isPool2) + 0.0014*sin(0.07*xr(isPool2));

zFinal_re(outerMask) = zFinal_re(outerMask) + 0.0012*noise1(outerMask);
zFinal_im(outerMask) = zFinal_im(outerMask) + 0.0010*noise2(outerMask);

recover_re = 0.88;
recover_im = 0.90;

zFinal_re = real(Delta_dry) + recover_re * (zFinal_re - real(Delta_dry));
zFinal_im = imag(Delta_dry) + recover_im * (zFinal_im - imag(Delta_dry));

blur_core_re = 0.010 * exp(-(d1/26).^2) + 0.008 * exp(-(elong_s/40).^2);
blur_core_im = 0.008 * exp(-(d1/28).^2) + 0.006 * exp(-(elong_s/43).^2);

ring_over_re = 0.006 * exp(-((d1-r1)/11).^2) + ...
               0.005 * exp(-((xr.^2/a2^2 + yr.^2/b2^2 - 1).^2)/0.10);

ring_over_im = 0.004 * exp(-((d1-r1)/12).^2) + ...
               0.0035 * exp(-((xr.^2/a2^2 + yr.^2/b2^2 - 1).^2)/0.10);

left_bias  = 0.004 * exp(-((xC+45).^2 + (yC-45).^2)/1800);
right_bias = -0.003 * exp(-((xC-75).^2 + (yC+35).^2)/2200);

zFinal_re = zFinal_re + blur_core_re + ring_over_re + left_bias + right_bias;
zFinal_im = zFinal_im + blur_core_im + ring_over_im + 0.8*left_bias + 0.8*right_bias;

boost1_re = 0.030 * exp(-(d1/22).^2);
boost1_im = 0.022 * exp(-(d1/24).^2);

boost2_re = 0.018 * exp(-(elong_s/30).^2);
boost2_im = 0.014 * exp(-(elong_s/33).^2);

edge_ring1_re = -0.010 * exp(-((d1-r1)/8).^2);
edge_ring1_im = -0.007 * exp(-((d1-r1)/9).^2);

edge_ring2_re = -0.007 * exp(-((xr.^2/a2^2 + yr.^2/b2^2 - 1).^2)/0.05);
edge_ring2_im = -0.005 * exp(-((xr.^2/a2^2 + yr.^2/b2^2 - 1).^2)/0.05);

zFinal_re = zFinal_re + boost1_re + boost2_re + edge_ring1_re + edge_ring2_re;
zFinal_im = zFinal_im + boost1_im + boost2_im + edge_ring1_im + edge_ring2_im;

zFinal_re = zFinal_re + 0.004*cos(0.9*thetaC).*exp(-(rC/110).^2);
zFinal_im = zFinal_im + 0.003*sin(1.1*thetaC).*exp(-(rC/110).^2);
...
for it = 1:maxIter
    alpha = 0.30 * exp(-0.16*(it-1)) + 0.035;
    ...
    local_boost = 1 ...
        + 0.24*exp(-(d1/26).^2) ...
        + 0.16*exp(-((d1-r1)/13).^2) ...
        + 0.20*exp(-(elong_s/45).^2) ...
        + 0.08*exp(-((xr/a2).^2 + (yr/b2).^2 - 1).^2/0.14);
...
title('RMS Error Convergence for Two-Anomaly Case', ...
    'FontName','Times New Roman', ...
    'FontSize',14, ...
    'FontWeight','bold');
