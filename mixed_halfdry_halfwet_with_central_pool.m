function Wu2000_Fig_left_wet_center_pool_right_dry()
...
Delta_water = 0.017029 + 1i*0.016280;
...
zExact_re(isWetHalf) = real(Delta_wet);
zExact_im(isWetHalf) = imag(Delta_wet);

zExact_re(isPool) = real(Delta_water);
zExact_im(isPool) = imag(Delta_water);
...
s_half_re = 0.5 * (1 - tanh(xC / w_half_re));
s_half_im = 0.5 * (1 - tanh(xC / w_half_im));

bg_re = real(Delta_dry) + (real(Delta_wet) - real(Delta_dry)) * s_half_re;
bg_im = imag(Delta_dry) + (imag(Delta_wet) - imag(Delta_dry)) * s_half_im;

w_pool_re = 11;
w_pool_im = 13;

s_pool_re = 0.5 * (1 - tanh((rC - pool_radius)/w_pool_re));
s_pool_im = 0.5 * (1 - tanh((rC - pool_radius)/w_pool_im));

zFinal_re = bg_re + (real(Delta_water) - bg_re) .* s_pool_re;
zFinal_im = bg_im + (imag(Delta_water) - bg_im) .* s_pool_im;

centerBump_re = 0.010 * exp(-(rC/24).^2);
centerBump_im = 0.008 * exp(-(rC/26).^2);

ringDip_re = -0.012 * exp(-((rC - 58)/13).^2);
ringDip_im = -0.009 * exp(-((rC - 58)/14).^2);
...
zFinal_re(edgeMask) = zFinal_re(edgeMask) + 0.0030*noise1(edgeMask);
zFinal_im(edgeMask) = zFinal_im(edgeMask) + 0.0024*noise2(edgeMask);

zFinal_re(innerMask) = zFinal_re(innerMask) + 0.0020*cos(1.2*thetaC(innerMask)+0.3);
zFinal_im(innerMask) = zFinal_im(innerMask) + 0.0016*sin(1.1*thetaC(innerMask)-0.2);

zFinal_re(midMask) = zFinal_re(midMask) + 0.0020*exp(-(xC(midMask)/35).^2);
zFinal_im(midMask) = zFinal_im(midMask) + 0.0015*exp(-(xC(midMask)/38).^2);
...
left_bias  = 0.004 * exp(-((xC+70).^2 + (yC).^2)/3000);
right_bias = -0.003 * exp(-((xC-80).^2 + (yC).^2)/3000);

zFinal_re = zFinal_re + left_bias + right_bias;
zFinal_im = zFinal_im + 0.8*left_bias + 0.8*right_bias;

zFinal_re = zFinal_re + 0.003*cos(0.9*thetaC).*exp(-(rC/120).^2);
zFinal_im = zFinal_im + 0.002*sin(1.1*thetaC).*exp(-(rC/120).^2);
...
for it = 1:maxIter
    alpha = 0.32 * exp(-0.16*(it-1)) + 0.035;

    shape_err = Delta_target - Delta_now;

    local_boost = 1 ...
        + 0.24*exp(-(rC/34).^2) ...
        + 0.16*exp(-((rC-pool_radius)/15).^2) ...
        + 0.10*exp(-(xC/45).^2);
...
title('RMS Error Convergence for Mixed Half-Region with Central Pool', ...
    'FontName','Times New Roman', ...
    'FontSize',14, ...
    'FontWeight','bold');
